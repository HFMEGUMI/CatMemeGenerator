...
class VideoGenerator:
    ...
    def load_asset_frame(self, asset_id, frame_number):
        # 转换帧率
        asset_frame = int(frame_number * (60/self.fps))
        frame_file = f"{asset_frame:05d}.png"
        frame_path = os.path.join("./memes", asset_id, "png", frame_file)
        
        # 若出错则使用最后一帧
        if not os.path.exists(frame_path):
            png_dir = os.path.join("./memes", asset_id, "png")
            available_frames = sorted(os.listdir(png_dir))
            if available_frames:
                frame_path = os.path.join(png_dir, available_frames[-1])
            else:
    ...
    def generate_frame(self, scene, frame_time):
        ...
        for fg in scene['foregrounds']:
            ...
            try:
                # 加载和缩放素材
                asset_img = self.load_asset_frame(asset_id, frame_number)
